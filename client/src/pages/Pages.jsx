...
export function LoginPage() {
  ...
  const submit = async (event) => { event.preventDefault(); const clientError = validateLoginForm(form); if (clientError) { setError(clientError); return; } setLoading(true); setError(null); try { const user = await login(form, role); const destination = user.role === "admin" ? "/admin" : user.role === "customer" ? "/dashboard" : "/provider"; navigate(location.state?.from?.pathname || destination, { replace: true }); } catch (nextError) { setError(nextError); } finally { setLoading(false); } };
...
export function RegisterPage() {
  ...
  const [form, setForm] = useState({ fullName: "", mobile: "", password: "", confirmPassword: "", categoryIds: [], experience: "", address: "" });
  ...
  const toggleCategory = (categoryId) => setForm({
    ...form,
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (role === "provider") {
        Object.assign(payload, { categoryIds: form.categoryIds, experience: Number(form.experience), address: form.address });
      }
...
function RoleSwitch({ value, onChange }) { return <div className="segmented"><button type="button" className={value === "customer" ? "active" : ""} onClick={() => onChange("customer")}>Customer</button><button type="button" className={value === "provider" ? "active" : ""} onClick={() => onChange("provider")}>Provider</button><button type="button" className={value === "admin" ? "active" : ""} onClick={() => onChange("admin")}>Admin</button></div>; }
...
export function CustomerDashboard() {
  ...
  const load = useCallback((initial = false) => { if (initial) setLoading(true); setError(null); Promise.all([initial ? api.get("/categories") : Promise.resolve(null), api.get(`/providers?page=1&limit=20${categoryId ? `&categoryId=${categoryId}` : ""}`)]).then(([categoriesResponse, providersResponse]) => { if (categoriesResponse) setCategories(categoriesResponse.data.data.categories); const nextProviders = providersResponse.data.data.providers; setProviders((current) => JSON.stringify(current) === JSON.stringify(nextProviders) ? current : nextProviders); }).catch(setError).finally(() => { if (initial) setLoading(false); }); }, [categoryId]);
  useEffect(() => { load(true); const interval = window.setInterval(() => load(), 5000); return () => window.clearInterval(interval); }, [load]);
...
export function ProviderDashboard() { const [data, setData] = useState(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(null); const [actionError, setActionError] = useState(null); const [notice, setNotice] = useState(""); const [cashLoading, setCashLoading] = useState(null); const load = () => { setLoading(true); setError(null); Promise.all([api.get("/bookings/provider/requests?page=1&limit=20"), api.get("/bookings/provider/active?page=1&limit=20"), api.get("/bookings/provider/history?page=1&limit=20")]).then(([requests, active, history]) => setData({ requests: requests.data.data.bookings, active: active.data.data.bookings, history: history.data.data.bookings })).catch(setError).finally(() => setLoading(false)); }; useEffect(load, []); const transition = async (id, action, body) => { setActionError(null); try { await api.patch(`/bookings/${id}/${action}`, body); load(); } catch (nextError) { setActionError(nextError); } }; const cash = async (bookingId) => { setCashLoading(bookingId); setActionError(null); setNotice(""); try { await api.post("/payments/cash/received", { bookingId }); setNotice("Cash receipt reported. Admin verification is pending."); load(); } catch (nextError) { setActionError(nextError); } finally { setCashLoading(null); } }; if (loading) return <LoadingState label="Loading your workboard" />; if (error) return <ErrorState error={error} onRetry={load} />; return <><PageHeader eyebrow="Provider workspace" title="Your workboard"><Link className="button button-light" to="/provider/profile">Profile settings</Link></PageHeader>{actionError && <ErrorState error={actionError} />}{notice && <Notice tone="success">{notice}</Notice>}<ProviderColumn title="New requests" bookings={data.requests} action={(booking) => <><button className="button button-small" onClick={() => transition(booking.id, "accept")}>Accept</button><button className="button button-light button-small" onClick={() => transition(booking.id, "reject", { rejectionReason: "Schedule unavailable" })}>Decline</button></>} /><ProviderColumn title="Active visits" bookings={data.active} action={(booking) => <>{booking.status === "accepted" && <button className="button button-small" onClick={() => transition(booking.id, "start")}>Start visit</button>}{booking.status === "in_progress" && <button className="button button-small" onClick={() => transition(booking.id, "complete")}>Complete visit</button>}</>} /><ProviderColumn title="History" bookings={data.history} action={(booking) => booking.status === "completed" && <button className="button button-light button-small" disabled={cashLoading === booking.id} onClick={() => cash(booking.id)}>{cashLoading === booking.id ? "Reporting..." : "Report cash received"}</button>} /></>; }
...
export function ProviderProfilePage() {
  ...
  const [error, setError] = useState(null);
  const load = () => unwrap(api.get("/providers/me"), setData, setError, setLoading);
  useEffect(load, []);
  const toggleAvailability = async () => {
    if (!data.profile.availability && !(Number(data.profile.startingCharge) > 0)) {
      setError({ response: { data: { message: "Starting charge must be set by an admin before this provider can become available." } } });
      return;
    }
    setSaving(true); setError(null);
    try { const response = await api.patch("/providers/me/availability", { availability: !data.profile.availability }); setData({ ...data, profile: { ...data.profile, availability: response.data.data.availability } }); }
  ...
  return <><PageHeader eyebrow="Provider profile" title={data.user.fullName}><span className="status-pill">{profile.approvalStatus}</span></PageHeader>{error && <ErrorState error={error} />}<section className="profile-panel"><div><span className="section-label">Category</span><strong>{data.categories?.map((category) => category.name).join(", ") || "Not set"}</strong></div><div><span className="section-label">Rating</span><strong>{Number(profile.ratingAverage || 0).toFixed(1)} / 5</strong></div><div><span className="section-label">Starting charge</span><strong><Money value={profile.startingCharge} /></strong></div><div><span className="section-label">Availability</span><strong>{profile.availability ? "Available" : "Unavailable"}</strong></div></section><section className="profile-panel"><p>Starting charge is managed by the platform admin.</p>{!profile.availability && !(Number(profile.startingCharge) > 0) && <p className="inline-error">Ask an admin to set a starting charge before becoming available.</p>}<button type="button" className="button button-light" disabled={saving || profile.approvalStatus !== "approved" || (!profile.availability && !(Number(profile.startingCharge) > 0))} onClick={toggleAvailability}>{profile.availability ? "Set unavailable" : "Set available"}</button></section></>;
}
